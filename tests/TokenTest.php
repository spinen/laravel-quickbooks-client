...
class TokenTest extends TestCase
{
    ...
    {
        $this->token->owner = Mockery::mock(User::class);
        ...
        $this->token->owner->shouldReceive('quickBooksToken')
                          ->once()
                          ->withNoArgs()
                          ->andReturn($token_mock);

        $this->token->owner->id = 1;
    ...
    {
        $this->assertEquals('App\User,user_id,id', $this->token->owner());
    }
}
